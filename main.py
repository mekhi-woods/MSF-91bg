...
def run_plotter(final_dir: str = 'plots/'):
    ...
    #                      label = False)
    # plotter.mu_v_z(path_91bg = pm_91bg_merged_cut, path_norm = pm_norm_merged_cut,
    #                save_loc = final_dir+'mu_v_z.png', label = False)
    #
    # # SALT3 Plots
    ...
    plotter.dust_mass_step(path_91bg=pm_91bg_merged_cut, path_norm=pm_norm_merged_cut, path_dust=pm_dust,
                           save_loc=final_dir + 'dust_mass_step.png', label=True)
    # plotter.dust_v_scatter(path_91bg=pm_91bg_merged_cut, path_norm=pm_norm_merged_cut, path_dust=pm_dust,
    #                        bin_width_91bg=0.5, bin_width_norm=0.5, binning_lim=2,
    #                        save_loc=final_dir + 'dust_v_scatter.png', label=True)
    # WIP
    # =================================================================================================================
...
def run_stats():
    import numpy as np
    pm_91bg_merged_cut = 'results/91bg_snpy-salt_params_cut.txt'
    tb_91bg = utils.default_open(pm_91bg_merged_cut, True)
    tb_91bg_snpy = tb_91bg[tb_91bg['algo'] == 'SNPY']
    tb_91bg_salt = tb_91bg[tb_91bg['algo'] == 'SALT']
    for root, tb in zip(["snpy", "salt"], [tb_91bg_snpy, tb_91bg_salt]):
        for cat in ['color', 'color_err', 'stretch', 'stretch_err']:
            # print("\\newcommand{\\"+f"{root}"+f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Min"+"}{"+f"{min(tb[cat]):.2f}"+"}")
            print("\\newcommand{\\"+f"{root}"+f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Med"+"}{"+f"{np.median(tb[cat]):.2f}"+"}")
            # print("\\newcommand{\\"+f"{root}"+f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Max"+"}{"+f"{max(tb[cat]):.2f}"+"}")
            print("\\newcommand{\\"+f"{root}"+f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Med"+"}{"+f"{np.median(tb[cat]):.2f}"+"}")

    pm_norm_merged_cut = 'results/norm_snpy-salt_params_cut.txt'
    tb_norm = utils.default_open(pm_norm_merged_cut, True)
    tb_norm_snpy = tb_norm[tb_norm['algo'] == 'SNPY']
    tb_norm_salt = tb_norm[tb_norm['algo'] == 'SALT']
    for root, tb in zip(["snpyNorm", "saltNorm"], [tb_norm_snpy, tb_norm_salt]):
        for cat in ['color', 'color_err', 'stretch', 'stretch_err']:
            print("\\newcommand{\\" + f"{root}" + f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Min" + "}{" + f"{min(tb[cat]):.2f}" + "}")
            print("\\newcommand{\\" + f"{root}" + f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Med" + "}{" + f"{np.median(tb[cat]):.2f}" + "}")
            print("\\newcommand{\\" + f"{root}" + f"{cat[0].upper()}{cat[1:].replace('_err', 'Err')}Max" + "}{" + f"{max(tb[cat]):.2f}" + "}")
...
if __name__ == '__main__':
    ...
    dataManipulation.selection_criteria(data_path='results/91bg_snpy-salt_params.txt',
                                        selection='snpy+salt',
                                        save_loc='results/91bg_snpy-salt_params_cut.txt',
                                        criteria=criteria_91bg)
    dataManipulation.selection_criteria(data_path='results/91bg_snpy-salt_params.txt',
                                        selection='snpy',
                                        save_loc='results/91bg_snpy_params_cut.txt',
                                        criteria=criteria_91bg)
    dataManipulation.selection_criteria(data_path='results/91bg_snpy-salt_params.txt',
                                        selection='salt',
                                        save_loc='results/91bg_salt_params_cut.txt',
                                        criteria=criteria_91bg)
    # dataManipulation.correct_average_dist_mod(snpy_path='results/91bg_snpy_params_cut.txt',
    #                                           salt_path='results/91bg_salt_params_cut.txt')
    # run_lc_checker('91bg')
    # =================================================================================================================
    # ###
    ...
    #     'x1':          [-3.0, 3.0], # Brout+Scholnic+21
    #     'x1_err':      [-999, 0.1]  # 1.0
    # }
    # dataManipulation.selection_criteria(data_path='results/norm_snpy-salt_params.txt',
    #                                     selection='snpy+salt',
    #                                     save_loc='results/norm_snpy-salt_params_cut.txt',
    #                                     criteria=criteria_norm)
    # dataManipulation.selection_criteria(data_path='results/norm_snpy-salt_params.txt',
    #                                     selection='snpy',
    #                                     save_loc='results/norm_snpy_params_cut.txt',
    #                                     criteria=criteria_norm)
    # dataManipulation.selection_criteria(data_path='results/norm_snpy-salt_params.txt',
    #                                     selection='salt',
    #                                     save_loc='results/norm_salt_params_cut.txt',
    #                                     criteria=criteria_norm)
    # run_lc_checker('norm')
    # =================================================================================================================
    ...
    # run_stats()
    # plotter.distance_modulus_residuals('results/91bg_snpy_params_cut.txt', 'results/91bg_salt_params_cut.txt')
